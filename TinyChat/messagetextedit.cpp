...
QVector<std::shared_ptr<MsgInfo>> MessageTextEdit::getMsgList() {
    ...
    for (int index = 0; index < doc.size(); index++)
    {
        ...
        if (doc[index] == QChar::ObjectReplacementCharacter)
        {
            if (!text.isEmpty())
            {
                insertMsgList(total_msg_list_, MsgType::TEXT_MSG, text, QPixmap(), "", 0, "");
                text.clear();
            }
            while (indexUrl < count)
            {
                std::shared_ptr<MsgInfo> msg = img_or_file_list_[indexUrl];
                if (this->document()->toHtml().contains(msg->_text_or_url, Qt::CaseSensitive))
                {
                    indexUrl++;
                    total_msg_list_.append(msg);
                    break;
                }
                indexUrl++;
            }
    ...
    img_or_file_list_.clear();
    this->clear();
    return total_msg_list_;
}
...
void MessageTextEdit::insertImages(const QString & url) {
    ...
    insertMsgList(img_or_file_list_, MsgType::IMG_MSG, url, QPixmap::fromImage(image), unique_name,
        total_size, fileMd5);
}
...
void MessageTextEdit::insertFiles(const QString& url) {
    ...
    QString origin_name = fileInfo.fileName();
    QString unique_name = generateUniqueFileName(origin_name);
    insertMsgList(img_or_file_list_, MsgType::FILE_MSG, url, pix, unique_name,
        total_size, fileMd5);
}
